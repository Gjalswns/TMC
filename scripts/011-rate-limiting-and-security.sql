-- =====================================================
-- 011: Rate Limiting and Security Enhancements
-- =====================================================
...
CREATE TABLE IF NOT EXISTS rate_limit_tracking (
  ...
  action VARCHAR(100) NOT NULL, -- 'join_game', 'submit_answer', etc.
...
CREATE INDEX idx_rate_limit_identifier_action ON rate_limit_tracking(identifier, action);
...
CREATE INDEX idx_abuse_identifier ON abuse_reports(identifier);
CREATE INDEX idx_abuse_type ON abuse_reports(abuse_type);
CREATE INDEX idx_abuse_severity ON abuse_reports(severity);
CREATE INDEX idx_abuse_detected ON abuse_reports(detected_at DESC);
CREATE INDEX idx_abuse_unresolved ON abuse_reports(resolved) WHERE NOT resolved;
...
BEGIN
  ...
  -- Check format (alphanumeric, Korean, spaces, hyphens, underscores)
...
CREATE INDEX idx_active_sessions_participant ON active_sessions(participant_id);
CREATE INDEX idx_active_sessions_game ON active_sessions(game_id);
CREATE INDEX idx_active_sessions_expires ON active_sessions(expires_at);
CREATE INDEX idx_active_sessions_active ON active_sessions(is_active) WHERE is_active;
...
CREATE OR REPLACE FUNCTION cleanup_old_data(p_days_to_keep INTEGER DEFAULT 30)
RETURNS JSON AS $$
DECLARE
...
-- MIGRATION COMPLETE
-- =========================
-- All security and rate limiting features have been implemented
-- Features:
