-- =====================================================
-- 014: Fix Function Parameter Names
-- =====================================================
-- Change parameter names to match Supabase RPC calls
-- =====================================================

-- Fix is_game_joinable function
...
BEGIN
  -- Get game information
  SELECT * INTO game_record FROM games g WHERE g.id = game_id;
  ...
  IF NOT is_game_joinable(game_id) THEN
    RETURN json_build_object('valid', false, 'error', 'Game is not available for joining');
  END IF;
  
  -- Check nickname uniqueness
  IF EXISTS (SELECT 1 FROM participants p WHERE p.game_id = game_id AND p.nickname = nickname) THEN
    RETURN json_build_object('valid', false, 'error', 'Nickname already taken in this game');
  END IF;
  
  -- Check student_id uniqueness (if provided)
  IF student_id IS NOT NULL AND EXISTS (
    SELECT 1 FROM participants p
    WHERE p.game_id = game_id AND p.student_id = student_id
  ) THEN
