...
BEGIN
    ...
    -- waiting 또는 in_progress 상태일 때만 참가 가능
    RETURN game_status IN ('waiting', 'in_progress');
END;
$$;
...
COMMENT ON FUNCTION is_game_joinable(UUID) IS '게임 참가 가능 여부 확인';
COMMENT ON FUNCTION get_game_by_code(TEXT) IS '게임 코드로 게임 정보 조회';
